...
def construct_representative(clusters, reads, monomers):
    res = []
    new_clusters = []
    for i in range(len(clusters)):
        ...
        if len(cl[best_ind]["seq"]) < 200:
            min_ed = 200
            for m in monomers:
                if len(m.seq) < len(cl[best_ind]["seq"]):
                    ed = edist_hw([m.seq, cl[best_ind]["seq"]])
                    min_ed = min(min_ed, min(ed, edist_hw([m.seq.reverse_complement(), cl[best_ind]["seq"]])))
                else:
                    ed = edist_hw([cl[best_ind]["seq"], m.seq])
                    min_ed = min(min_ed, min(ed, edist_hw([cl[best_ind]["seq"], m.seq.reverse_complement()])))
            if min_ed < 0.25*len(cl[best_ind]["seq"]):
                alsat = "CM"
            else:
                print(len(cl[best_ind]["seq"]), min_ed)
        if len(alsat) == 0:
...
def identify_nm(reads_regions, params, reads, monomers):
    ...
    consensus, clusters = construct_representative(clusters, reads, monomers)
    total = len(consensus)
