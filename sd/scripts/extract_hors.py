...
def convert_dec_to_internal_monomers(reads_mapping, monomers):
    monomers_lst = sorted([x for x in monomers], key=lambda x: int(x.split(".")[1].split("/")[0])) #sorted([x.split("(")[0] for x in monomers], key=lambda x: ord(x[0])) #
    monomers_mp = {}
    monomers_mp_r = {}
            ...
            for c in seq[r]:
                if c["qid"] == "NM":
                    ...
                    if c["qid"] in monomers_mp:
                        fout.write("\t".join([r, convert_to_initial_mono(c["qid"], monomers_mp), str(c["s"]), str(c["e"]), "{0:.2f}".format(c["idnt"]),\
                                              str(c["len"]) if not c["qid"].startswith("f") else "-1", \
                                              str(c["e"] - c["s"] + 1), str(c["s"] - prev)]) + "\n")
                    else:
                        fout.write("\t".join([r, convert_to_ivan_hors(c["monomers_lst"], monomers_mp, known_hors_initial), str(c["s"]), str(c["e"]), "{0:.2f}".format(c["idnt"]), \
