    ...
    void AlignReadsSet(vector<Seq> &reads, int threads, int part_size) {
        ...
        for (int i = 0; i < new_reads.size(); i += step) {
            ...
            for (int j = i; j < min(i + step, (int) new_reads.size()); ++ j) {
                ...
                {
                    if (i % 1000 == 0) {
                        cerr << "Prepared reads: " << i << " " << j << " " << i + step << " " << new_reads.size() << "\n";
                        cerr << "Filtered monomers number: " << filter_monomers.size() << "\n";
                    }
                    subbatches.push_back(pair<int, vector<MonomerAlignment>> (j, aln));
                }
            }
            cerr << "Sort subbatches\n";
...
private:
    ...
    vector<MonomerAlignment> AlignPartClassicDP(Seq &read, std::vector<Seq>& monomers) {
        ...
        for (int i = 1; i < read.seq.size(); ++ i) {
            ...
            for (int j = 0; j < monomers.size(); ++ j) {
                for (int k = 0; k < monomers[j].size(); ++ k) {
                    int score = INF;
